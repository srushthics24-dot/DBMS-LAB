...
select name from Person p, Owns o
 where (select count(reg_num) 
 from Participated d 
 where d.reg_num = o.reg_num and o.driver_id = p.driver_id) > 1;

select c.model 
from Car c 
where NOT EXISTS (select 1 from Participated p where p.reg_num = c.reg_num);

select * from Accident 
ORDER BY accident_date DESC limit 1;

select p.name, avg(d.damage_amount) "Average Damage"
 from Person p, Participated d 
 where p.driver_id = d.driver_id 
 group by d.driver_id;

update Participated p, Car c set p.damage_amount = 25000 
where p.reg_num = c.reg_num and c.model = 'Audi';

select * from Participated;

select p.name, (d.damage_amount) "Damage"
 from Person p, Participated d 
 where p.driver_id = d.driver_id
 order by d.damage_amount DESC limit 1;

select c.model, SUM(p.damage_amount) "Damage" 
